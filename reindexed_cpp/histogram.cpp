...
void Histogram::Fill_WvsmmSQ_2pi(double W, double W_dpp, double delta_zero_, double rho_, double mmSQ, int sec_number, float wt) {
        if (sec_number == sec_number && sec_number >= 0 && sec_number < 7) {
...
void Histogram::Fill_theta_P_inv_mass(float inv_mass, float theta, float wt) { theta_P_vs_mass_pip_pim->Fill(inv_mass, theta, wt); }
void Histogram::Fill_theta_pim_inv_mass(float inv_mass, float theta, float wt) { theta_pim_vs_mass_Ppip->Fill(inv_mass, theta, wt); }
void Histogram::Fill_theta_pip_inv_mass(float inv_mass, float theta, float wt) { theta_pip_vs_mass_Ppim->Fill(inv_mass, theta, wt); }
