...
void datahandeler(std::string fin, std::string fout) {
  ...
  for (int current_event = 0; current_event < num_of_events; current_event++) {
    ...
    if (event->e_mu_prime().P() != 0) {
      hist->Fill_EC_sampling_fraction(event->e_mu_prime().P(), (ec_tot_energy->at(0) / event->e_mu_prime().P()), sector,
                                      mc_weight);
    ...
    for (int part = 1; part < pid->size(); part++) {
      ...
      if                            /*(abs(dt->dt_Pi()) < 0.5 || abs(dt->dt_ctof_Pi()) < 0.5) /*&&*/
        ...
        good_pim = e_cuts->pim_cuts(status->at(part), charge->at(part), event->pim_mu_prime().P(), pid->at(part),
                                    chi2pid->at(part));
        good_hadron_ctof_pim = e_cuts->hadron_cuts_ctof(status->at(part), charge->at(part), event->pim_mu_prime().P(),
      ...
      if (pid->at(part) == PROTON) {  // cut 9
        ...
        if (abs(dt->dt_ctof_P()) < 10.1) {
          ...
          //  if (abs(dt->dt_ctof_P()) < 0.5 && good_hadron_ctof_P) {
          no_proton_ctof += 1;
      ...
      if (pid->at(part) == PIP) {  // cut 9
        ...
        good_pip = e_cuts->pip_cuts(status->at(part), charge->at(part), event->pip_mu_prime().P(), pid->at(part),
                                    chi2pid->at(part));
        good_hadron_ctof_pip = e_cuts->hadron_cuts_ctof(status->at(part), charge->at(part), event->pip_mu_prime().P(),
                                                        pid->at(part), chi2pid->at(part));
        ...
        if (abs(dt->dt_ctof_Pi()) < 10.1) {
          ...
          // if ((dt->dt_ctof_Pi() < 0.5) && (dt->dt_ctof_Pi() > -0.30) && (good_hadron_ctof_pip)){
    ...
    hist->Fill_pid_size_fn(pid->size(), (pid_check));
    ...
    if (/*event->W() < 1.40 &&  event->W() > 1.20 &&*/ event->Q2() < 15.0 && event->Q2() > 0.0 &&
      ...
      //
      // } else
      if (event->twoPionEvent()) {  // cut # 13

        hist->Fill_2pion_mm(event->MM(), sector, mc_weight);
        hist->Fill_2pion_mmSQ(event->MM2(), sector, mc_weight);
        // if (0.18< ec_tot_energy->at(0) / event->e_mu_prime().P() && ec_tot_energy->at(0) /
        // event->e_mu_prime().P()< 0.28) {
        //
        ...
        if (event->MM2() < 0.03 && event->MM2() > -0.03) {  // cut # 14
          ...
          hist->Fill_theta_P_inv_mass(event->W_rho(), (event->p_mu_prime_cm().Theta() * (180 / PI)), mc_weight);
          ...
          hist->Fill_theta_P_lab_inv_mass(event->W_rho(), (event->p_mu_prime().Theta() * (180 / PI)), mc_weight);
  ...
  std::cout << "no_of_ele  " << no_ele << " pim  " << no_pim << " pip  " << no_pip << " proton " << no_proton
            << "  e_as_well_pim " << no_e_as_well_pim << "  p_as_well_pip " << no_p_as_well_pip << '\n';
  std::cout << "pid size 1 " << pid_size_1 << "2 " << pid_size_2 << "  size_3 " << pid_size_3 << "  size_4 "
            << pid_size_4 << "  size_5 " << pid_size_5 << "  size_6 " << pid_size_6 << "  size_7_or_greater "
            << pid_size_7 << '\n';
  std::cout << "pr_ftof " << no_proton_ftof << " pr_ctof " << no_proton_ctof << " pip_ftof " << no_pip_ftof
            << " pip_ctof " << no_pip_ctof << " pim_ftof " << no_pim_ftof << " pim_ctof " << no_pim_ctof << '\n';
