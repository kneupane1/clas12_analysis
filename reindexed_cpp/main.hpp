    ...
    for (int part = 1; part < pid->size(); part++) {
      ...
      if (/*event->W() < 1.40 &&  event->W() > 1.20 &&*/ event->Q2() < 15.0 && event->Q2() > 0.0) {
        if (charge->at(part) == -1) {
          ...
          hist->Fill_deltat_kp(pid->at(part), charge->at(part), dt->dt_K(), p->at(part));
          //    if (charge->at(part) == 1) {
          if ((abs(dt->dt_P()) < 0.5) || (dt->dt_P() > -4.5 && dt->dt_P() < -3.7) && (pid->at(part) == 2112)) {
...
event->SetPim(px->at(part), py->at(part), pz->at(part), MASS_PIP);
good_pim =
    e_cuts->pim_cuts(status->at(part), charge->at(part), event->pim_mu_prime().P(), pid->at(part), chi2pid->at(part));
if (pid->at(part) == PIM) {
  cc_tot_pim = cc_nphe_tot->at(part);
  if (cc_tot_pim >= 0) {
    hist->Fill_hist_cc_tot_pim(cc_tot_pim);
...
else event->SetOther(px->at(part), py->at(part), pz->at(part), MASS_N, pid->at(part));
}
}
...
if (/*event->W() < 1.40 &&  event->W() > 1.20 &&*/ event->Q2() < 15.0 && event->Q2() > 0.0) {
  ...
  if (event->elecProtEvent()) {
    ...
    if (event->MM2() < 0.1 && event->MM2() > -0.1) {
      hist->Fill_WvsmmSQ_2pi(event->W_2pi(), event->W_delta_pp(), event->W_delta_zero(), event->W_rho(), event->MM2(),
                             sector);
    } else {
