                        ...
                        for (int part = 1; part < pid->size(); part++) {
                                ...
                                if (/*event->W() < 1.40 &&  event->W() > 1.20 &&*/ event->Q2() < 15.0 && event->Q2() > 0.0) {         // cut # 5
                                        ...
                                        if (charge->at(part) == -1) {         // cut # 6
                                                ...
                                                if ((abs(dt->dt_Pi()) < 0.50) || (dt->dt_ctof_Pi() < 0.5 && dt->dt_ctof_Pi() > -0.3))
                                                {         // cut 9
                                ...
                                if (event->twoPionEvent()) {         // cut # 13
                                        if (event->e_mu_prime().P() != 0) {
                                                ...
                                                hist->Fill_EC_sampling_fraction(event->e_mu_prime().P(), (ec_tot_energy->at(0) / event->e_mu_prime().P()),
                                                                                sector, mc_weight );
                                                hist->Fill_PCAL_VS_ECAL(ec_pcal_energy->at(0), (ec_ecin_energy->at(0) + ec_ecout_energy->at(0)), sector, mc_weight);
                                        ...
                                        // if (0.18< ec_tot_energy->at(0) / event->e_mu_prime().P() && ec_tot_energy->at(0) / event->e_mu_prime().P()< 0.28) {
                                        //
                                        ...
                                        if (event->MM2() < 0.03 && event->MM2() > -0.03) {         // cut # 14
                                                ...
                                                hist->Fill_W_2pi_all_sec(event->W_2pi(), event->W_delta_pp(), event->W_delta_zero(), event->W_rho(),mc_weight);

                                                hist->Fill_theta_P_inv_mass(event->W_rho(), (event->p_mu_prime_cm().Theta() * (180 / PI)),mc_weight);

                                                hist->Fill_theta_pim_inv_mass(event->W_delta_pp(), (event->pim_mu_prime_cm().Theta() * (180 / PI)),mc_weight);
                                                hist->Fill_theta_pip_inv_mass(event->W_delta_zero(), (event->pip_mu_prime_cm().Theta() * (180 / PI)),mc_weight);

                                                hist->Fill_theta_P_lab_inv_mass(event->W_rho(), (event->p_mu_prime().Theta() * (180 / PI)),mc_weight);
                                                hist->Fill_theta_pim_lab_inv_mass(event->W_delta_pp(), (event->pim_mu_prime().Theta() * (180 / PI)),mc_weight);
                                                hist->Fill_theta_pip_lab_inv_mass(event->W_delta_zero(), (event->pip_mu_prime().Theta() * (180 / PI)),mc_weight);
                                        } else {
