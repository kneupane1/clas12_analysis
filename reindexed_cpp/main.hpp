...
void datahandeler(std::string fin, std::string fout) {
  ...
  for (int current_event = 0; current_event < num_of_events; current_event++) {
    ...
    if (mc_pid->size() != 0 && mc_pid->at(0) == ELECTRON /* charge->at(0) >= 0*/) {  // cut # 1
      ...
      hist->Fill_Angles_thrown(
          event->W_thrown(), event->Q2_thrown(), event->p_mu_prime_cm_thrown().Theta() * 180 / PI,
          event->pip_mu_prime_cm_thrown().Theta() * 180 / PI, event->pim_mu_prime_cm_thrown().Theta() * 180 / PI,
          event->p_mu_prime_cm_thrown().Phi() * 180 / PI, event->pip_mu_prime_cm_thrown().Phi() * 180 / PI,
          event->pim_mu_prime_cm_thrown().Phi() * 180 / PI, event->alpha_pippim_pipf_thrown(),
          event->alpha_ppim_pipip_thrown(), event->alpha_ppip_pipim_thrown(), mc_weight);
    }
    if (pid->size() != 0 && pid->at(0) == ELECTRON) {  ///* charge->at(0) >= 0*/) continue; // cut # 1
      ...
      good_e = e_cuts->electron_cuts(status->at(0), charge->at(0), (ec_tot_energy->at(0) / event->e_mu_prime().P()),
                                     ...
                                     sector, ec_pcal_x->at(0), ec_pcal_y->at(0), dc_r1_x->at(0), dc_r1_y->at(0),
                                     dc_r2_x->at(0), dc_r2_y->at(0), dc_r3_x->at(0), dc_r3_y->at(0));
      ...
      if (good_e) {  // cut # 2
        ...
        if (event->e_mu_prime().P() != 0) {
          ...
          if (event->e_mu_prime().P() > 3.7 && event->e_mu_prime().P() < 4.0)
            hist->Fill_1d_sampling_fraction_10(event->e_mu_prime().P(),
                                               (ec_tot_energy->at(0) / event->e_mu_prime().P()), sector, mc_weight);
          if (event->e_mu_prime().P() > 4.0 && event->e_mu_prime().P() < 4.3)
            hist->Fill_1d_sampling_fraction_11(event->e_mu_prime().P(),
                                               (ec_tot_energy->at(0) / event->e_mu_prime().P()), sector, mc_weight);
          if (event->e_mu_prime().P() > 4.3 && event->e_mu_prime().P() < 4.6)
            hist->Fill_1d_sampling_fraction_12(event->e_mu_prime().P(),
                                               (ec_tot_energy->at(0) / event->e_mu_prime().P()), sector, mc_weight);

          if (event->e_mu_prime().P() > 4.6 && event->e_mu_prime().P() < 4.9)
            hist->Fill_1d_sampling_fraction_13(event->e_mu_prime().P(),
                                               (ec_tot_energy->at(0) / event->e_mu_prime().P()), sector, mc_weight);

          if (event->e_mu_prime().P() > 4.9 && event->e_mu_prime().P() < 5.2)
            hist->Fill_1d_sampling_fraction_14(event->e_mu_prime().P(),
                                               (ec_tot_energy->at(0) / event->e_mu_prime().P()), sector, mc_weight);
          if (event->e_mu_prime().P() > 5.2 && event->e_mu_prime().P() < 5.5)
            hist->Fill_1d_sampling_fraction_15(event->e_mu_prime().P(),
                                               (ec_tot_energy->at(0) / event->e_mu_prime().P()), sector, mc_weight);
          if (event->e_mu_prime().P() > 5.5 && event->e_mu_prime().P() < 5.8)
            hist->Fill_1d_sampling_fraction_16(event->e_mu_prime().P(),
                                               (ec_tot_energy->at(0) / event->e_mu_prime().P()), sector, mc_weight);

          if (event->e_mu_prime().P() > 5.8 && event->e_mu_prime().P() < 6.1)
            hist->Fill_1d_sampling_fraction_17(event->e_mu_prime().P(),
                                               (ec_tot_energy->at(0) / event->e_mu_prime().P()), sector, mc_weight);
          if (event->e_mu_prime().P() > 6.1 && event->e_mu_prime().P() < 6.4)
            hist->Fill_1d_sampling_fraction_18(event->e_mu_prime().P(),
                                               (ec_tot_energy->at(0) / event->e_mu_prime().P()), sector, mc_weight);
          if (event->e_mu_prime().P() > 6.4 && event->e_mu_prime().P() < 6.7)
            hist->Fill_1d_sampling_fraction_19(event->e_mu_prime().P(),
                                               (ec_tot_energy->at(0) / event->e_mu_prime().P()), sector, mc_weight);
          if (event->e_mu_prime().P() > 6.7 && event->e_mu_prime().P() < 7.0)
            hist->Fill_1d_sampling_fraction_20(event->e_mu_prime().P(),
                                               (ec_tot_energy->at(0) / event->e_mu_prime().P()), sector, mc_weight);
        ...
        for (int part = 1; part < pid->size(); part++) {
          ...
          if (/*event->W() < 1.40 &&  event->W() > 1.20 &&*/ event->Q2() < 15.0 && event->Q2() > 0.0) {  // cut # 5
            ...
            if (charge->at(part) == -1) {  // cut # 6
              ...
              //         hist->Fill_ctof_pim_without_cut_hist(pid->at(part), charge->at(part), dt->dt_ctof_Pi(),
              //         p->at(part));
              //
              //         if (abs(dt->dt_ctof_Pi()) < 0.5) {
              //                 hist->Fill_ctof_pim_with_cut_hist(pid->at(part), charge->at(part), dt->dt_ctof_Pi(),
              //                 p->at(part));
              //         }
              // }
              if ((abs(dt->dt_Pi()) < 0.5 || abs(dt->dt_ctof_Pi()) < 0.5) && (pid->at(part) == PIM)) {  // cut # 8
                ...
                good_pim = e_cuts->pim_cuts(status->at(part), charge->at(part), event->pim_mu_prime().P(),
              ...
              //         hist->Fill_ctof_km_with_cut_hist(pid->at(part), charge->at(part), dt->dt_ctof_K(),
              //         p->at(part));
              // }
              ...
              //         hist->Fill_ctof_P_without_cut_hist(pid->at(part), charge->at(part), dt->dt_ctof_P(),
              //         p->at(part)); if (abs(dt->dt_ctof_P()) < 0.5) {
              //                 hist->Fill_ctof_P_with_cut_hist(pid->at(part), charge->at(part), dt->dt_ctof_P(),
              //                 p->at(part));
              //         }
              // }
              ...
              //         hist->Fill_ctof_pip_without_cut_hist(pid->at(part), charge->at(part), dt->dt_ctof_Pi(),
              //         p->at(part)); if ((dt->dt_ctof_Pi() < 0.5) && (dt->dt_ctof_Pi() > -0.30)) {
              //                 //(dt->dt_ctof_Pi() >= (0.67 * p->at(part) - 4.5)))
              //                 hist->Fill_ctof_pip_with_cut_hist(pid->at(part), charge->at(part), dt->dt_ctof_Pi(),
              //                 p->at(part));
              //         }
              // }
              if ((abs(dt->dt_P()) < 0.5 || abs(dt->dt_ctof_P()) < 0.5)
                ...
                good_hadron_ctof_P = e_cuts->hadron_cuts_ctof(
                    status->at(part), charge->at(part), event->p_mu_prime().P(), pid->at(part), chi2pid->at(part));
                ...
                good_pip = e_cuts->pip_cuts(status->at(part), charge->at(part), event->pip_mu_prime().P(),
                                            pid->at(part), chi2pid->at(part));
              ...
              //                                                             (pid->at(part) == KP)*/) {
              //         hist->Fill_ctof_kp_with_cut_hist(pid->at(part), charge->at(part), dt->dt_ctof_K(),
              //         p->at(part));
              // }
              // //}
              //
        ...
        // if ((good_pip || good_hadron_ctof_pip) && event->pip_mu_prime().Theta() != 0 &&
        ...
        // }
        // if ((good_pim || good_hadron_ctof_pim) && event->pim_mu_prime().Theta() != 0 && event->pim_mu_prime().P() !=
        // 0) {
        ...
        if (event->W() < 3.50 && event->W() > 0.0 && event->Q2() < 15.0 && event->Q2() > 0.0 &&
          ...
          if (event->elecProtEvent()) {
            ...
            if (event->MM2() < 0.01 && event->MM2() > -0.01) {  // cut # 14
              ...
              hist->Fill_Angles(event->W(), event->Q2(), event->p_mu_prime_cm().Theta() * 180 / PI,
                                event->pip_mu_prime_cm().Theta() * 180 / PI,
                                event->pim_mu_prime_cm().Theta() * 180 / PI, event->p_mu_prime_cm().Phi() * 180 / PI,
                                event->pip_mu_prime_cm().Phi() * 180 / PI, event->pim_mu_prime_cm().Phi() * 180 / PI,
                                event->alpha_pippim_pipf(), event->alpha_ppim_pipip(), event->alpha_ppip_pipim(),
                                mc_weight);
            } else {
          ...
          if (event->elecWopEvent()) {
            ...
            if (event->MM_wop() < 1.05 && event->MM2_wop() > 0.65) {
              // hist->Fill_deltat_after_mmsq_cut_Xpip(pid_of_part, charge_at_part, dt->dt_P(), dt->dt_Pi(),
              // mom_at_part); hist->Fill_deltat_ctof_after_mmsq_cut_Xpip(pid_of_part, charge_at_part, dt->dt_ctof_P(),
              // dt->dt_ctof_Pi(),
              //                                           mom_at_part);
