...
TLorentzVector Reaction::e_mu_prime_thrown() { return *_elec_thrown; }  // maile thapeko
...
void Reaction::AlphaCalc() {
  ...
  if (twoPionEvent()) {
    *_q_cm = (physics::boost_((*_beam - *_elec), *_beam, *_elec));
    ...
    if (Vect3_gamma.Cross(Vect3_beta) * _pim_mu_prime_cm_.Vect() < 0) alpha_PPIp_piPIm = 360. - alpha_PPIp_piPIm;
    ...
    if (Vect3_gamma.Cross(Vect3_beta) * _p_mu_prime_cm_.Vect() < 0) alpha_PIpPIm_pipf = 360. - alpha_PIpPIm_pipf;
    //α[pp'][π+π−]
    ...
    if (Vect3_gamma.Cross(Vect3_beta) * _pip_mu_prime_cm_.Vect() < 0) alpha_PPIm_piPIp = 360. - alpha_PPIm_piPIp;
    //α[pπ−][p'π+]
    //  return;
...
float Reaction::alpha_pippim_pipf() {  // alpha P
  return _alpha_pippim_pipf;
}
float Reaction::alpha_ppim_pipip() {  // alpha pip
...
void Reaction::AlphaCalc_thrown() {
  ...
  *_q_cm_thrown = (physics::boost_((*_beam - *_elec_thrown), *_beam, *_elec_thrown));
  ...
  // 1 this one is used for α[π−]
  a_gamma_thrown = sqrt(
      1. / (1 - pow((_pim_mu_prime_cm_thrown_.Vect().Unit() * V3_anti_z_thrown), 2)));  // V3_anti_z_thrown(0,0,-1);
  ...
  a_beta_thrown =
      sqrt(1. / (1 - pow((_pim_mu_prime_cm_thrown_.Vect().Unit() * _pip_mu_prime_cm_thrown_.Vect().Unit()), 2)));
  b_beta_thrown = -(_pim_mu_prime_cm_thrown_.Vect().Unit() * _pip_mu_prime_cm_thrown_.Vect().Unit()) * a_beta_thrown;
  Vect3_beta_thrown =
      a_beta_thrown * _pip_mu_prime_cm_thrown_.Vect().Unit() + b_beta_thrown * _pim_mu_prime_cm_thrown_.Vect().Unit();
  ...
  a_beta_thrown =
      sqrt(1. / (1 - pow((_p_mu_prime_cm_thrown_.Vect().Unit() * _pip_mu_prime_cm_thrown_.Vect().Unit()), 2)));
  b_beta_thrown = -(_p_mu_prime_cm_thrown_.Vect().Unit() * _pip_mu_prime_cm_thrown_.Vect().Unit()) * a_beta_thrown;
  Vect3_beta_thrown =
      a_beta_thrown * _pip_mu_prime_cm_thrown_.Vect().Unit() + b_beta_thrown * _p_mu_prime_cm_thrown_.Vect().Unit();
  ...
  a_beta_thrown =
      sqrt(1. / (1 - pow((_pip_mu_prime_cm_thrown_.Vect().Unit() * _pim_mu_prime_cm_thrown_.Vect().Unit()), 2)));
  b_beta_thrown = -(_pip_mu_prime_cm_thrown_.Vect().Unit() * _pim_mu_prime_cm_thrown_.Vect().Unit()) * a_beta_thrown;
  Vect3_beta_thrown =
      a_beta_thrown * _pim_mu_prime_cm_thrown_.Vect().Unit() + b_beta_thrown * _pip_mu_prime_cm_thrown_.Vect().Unit();
